...
RHEL major/minor:
1. `lightspeed-mcp` -> `get_rhel_lifecycle`
2. `WebFetch` -> `https://access.redhat.com/product-life-cycles/?product=Red%20Hat%20Enterprise%20Linux`

RHEL in user's inventory:
1. `lightspeed-mcp` -> `get_relevant_rhel_lifecycle`
2. `lightspeed-mcp` -> `list_hosts` + RHEL lifecycle page above

App Streams (Node.js, PostgreSQL, .NET, etc.):
1. `lightspeed-mcp` -> `get_appstreams_lifecycle`
2. `WebFetch` -> `https://access.redhat.com/support/policy/updates/errata/`
