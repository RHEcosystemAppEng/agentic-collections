...
JUDGE_FILE="$SCRIPT_DIR/llm_judge.py"

if [ ! -f "$TEST_FILE" ]; then
    echo "ERROR: Could not find test_outputs.py at $TEST_FILE"
...
ls -la /root/*.json 2>/dev/null || echo "No JSON files found"

# ── Phase 1: Deterministic pytest ─────────────────────────────────────
echo ""
echo "════════════════════════════════════════════"
